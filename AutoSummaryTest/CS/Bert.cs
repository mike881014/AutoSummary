...
namespace AutoSummaryTest
{
    class Bert
    {
        ...
        public Bert(string system_path)
        {
        ...
        public void LoadVector(ref List<string[]> vec_list)
        {
            ...
            using (StreamReader sr = new StreamReader(this.system_path + @"bert/Sentence_Seg.txt.vec"))
            {
        ...
        {
            //指定要寫入的路徑，並用big5編碼
            using (StreamWriter sw = new StreamWriter(system_path + @"bert\run.bat", false, System.Text.Encoding.GetEncoding("big5")))
            {
                /* 若在win底下，且有python2 和 python3的版本，那須改成"py rum.py"
                 * 若只有單一個python2而已，則可以改"python rum.py" ，也可以不改，因為沒啥差
                 * 在rum.py的第一行已改#! python2，若在linux家族底下，則可以改自行安裝的python2的位置  該行所講內容詳情請參考：https://python.freelycode.com/contribution/detail/139
                */
                sw.WriteLine($@"cd {system_path}bert");
                sw.WriteLine("py sentence_embedding.py");          //用bat檔下執行python檔案指令
            }

            //IIS 7以上如何執行執行檔 exe, bat，參考網站:http://cattoncareer.blogspot.com/2015/06/iis-7-exe-bat.html
            ProcessStartInfo pInfo = new System.Diagnostics.ProcessStartInfo(system_path + @"bert\run.bat");
        ...
        public List<string> GetConfig()
        {
            ...
            using (StreamReader sr = new StreamReader($@"{system_path}bert\config.txt"))
            {
                string str;
                while ((str = sr.ReadLine()) != null)
                {
                    config.Add(str.Replace(" ", "").Split('=')[1]);
                }
            }
            return config;
        }
